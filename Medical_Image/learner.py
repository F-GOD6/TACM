...
class Conv_Standard(nn.Module):
    ...
    # def channel_shuffle(self, hidden, label,x2s,y2s, shuffle_dict, shuffle_channel_id, shuffle = True):
    ...
    #         cur_class_2 = x2s[label == shuffle_dict[i]]
    ...
    def channel_shuffle(self, hidden, label, x2s, y2s, shuffle_dict, shuffle_channel_id, shuffle = True):
        num_channels = hidden.shape[1]
        new_data = []
        new_labels = [] #改标签
        # 如果未提供 channel_mask，则生成一个全局通道掩码
        if self.channel_mask is None:
            beta_distribution = torch.distributions.Beta(2, 2)
            beta_samples = beta_distribution.sample((num_channels,))
            self.channel_mask = beta_samples > 0.5  # 布尔掩码，形状为 (num_channels,)

        for i in range(self.args.num_classes):
            cur_class_1 = hidden[label == i]
            cur_class_2 = x2s[label == shuffle_dict[i]]

            num_samples = cur_class_1.shape[0]

            # 调整 cur_class_2 的样本数量以匹配 cur_class_1
            if cur_class_2.shape[0] < num_samples:
                indices = torch.randint(0, cur_class_2.shape[0], (num_samples,))
                cur_class_2 = cur_class_2[indices]
            elif cur_class_2.shape[0] > num_samples:
                cur_class_2 = cur_class_2[:num_samples]

            # 调整 channel_mask 的形状以便广播
            channel_mask_expanded = self.channel_mask.unsqueeze(0).to(cur_class_2.device)  # (1, num_channels)
            while channel_mask_expanded.dim() < cur_class_1.dim():
                channel_mask_expanded = channel_mask_expanded.unsqueeze(-1)  # 最终形状 (1, num_channels, 1, 1)（假设有空间维度）

            # 使用相同的通道掩码替换通道
            cur_class_new = torch.where(channel_mask_expanded, cur_class_2, cur_class_1)

            new_data.append(cur_class_new)
            new_labels.append(torch.full((cur_class_1.shape[0],), shuffle_dict[i], dtype=label.dtype, device=label.device))
        new_data = torch.cat(new_data, dim=0)
        ...
        if shuffle:
            indexes = torch.randperm(new_data.shape[0])
            new_data = new_data[indexes]
            new_labels = new_labels[indexes]
        else:
            indexes = torch.arange(new_data.shape[0])

        return new_data, new_labels, indexes
    ...
    def functional_forward_cf(self, hidden, label,x2s,y2s, sel_layer, shuffle_list, shuffle_channel_id, weights,
                                           is_training=True):
        ...
        for layer in range(4):
            if layer == sel_layer:
                hidden, label_new, _ = self.channel_shuffle(hidden, label,x2s,y2s, shuffle_list, shuffle_channel_id)
    ...
    def augmentation(self, x1s, y1s, x2s, y2s, lam_mix = 0,methods=None):
        if lam_mix == 0:
    ...
    def MLQA(self, x1s, y1s, x2s, y2s, x1q, y1q, x2q, y2q, lam_mix, task_2_shuffle_id, shuffle_list, shuffle_channel_id):
        ...
        if self.args.mix and self.args.aug:
        ...
        lamda = 0
        # x1s, y1s, indexes = self.channel_shuffle(x1s, y1s,x2s,y2s, shuffle_list, shuffle_channel_id)
        x1q, y1q, _ = self.channel_shuffle(x1q, y1q,x2q,y2q, shuffle_list, shuffle_channel_id)
        # x1s, lamda = self.augmentation(x1s, y1s, x2s, y2s, lam_mix,methods)
        # x1q, lamda = self.augmentation(x2q, y2q, x1q, y2s, lam_mix,methods)
        query_label = y1q
        support_label = y2q
        # x1q = torch.cat((x1q, x1s))
        # q_label = torch.cat((query_label, y1s))
        # support_label = torch.cat((support_label, y2s))
        self.channel_mask = None
        return x1q, query_label, support_label, lamda
    ...
    def functional_forward_MLQA(self, x1s, y1s, x2s, y2s, x1q, y1q, x2q, y2q, sel_layer, shuffle_list, shuffle_channel_id, weights, is_training=True):
        ...
        x1q = x1q.view(x1q.size(0), -1)

        x = F.linear(x1q, weights['logits.weight'], weights['logits.bias'])

        return x, new_label, support_label, lamda    
    def QA(self, x1s, y1s, x2s, y2s, x1q, y1q, x2q, y2q, lam_mix, task_2_shuffle_id, shuffle_list, shuffle_channel_id):

        kshot = int(x1s.shape[0] // self.args.num_classes)
        task_2_shuffle = np.array(
            [np.arange(kshot) + task_2_shuffle_id[idx] * kshot for idx in
            range(self.args.num_classes)]).flatten()
        da_pool = ['re', 'tlr', 're + tlr']
        methods = random.sample(da_pool, 1)[0]
        if methods == 're':
            x1s = self.random_erase(x1s)
        elif methods == 'tlr':
        ...
        return x1q, q_label, support_label, 0
    def functional_forward_QA(self, x1s, y1s, x2s, y2s, x1q, y1q, x2q, y2q, sel_layer, shuffle_list, shuffle_channel_id, weights, is_training=True):
        lam_mix = self.dist.sample().to(x1s.device)
        task_2_shuffle_id = np.arange(self.args.num_classes)
        np.random.shuffle(task_2_shuffle_id)
        ...
        for layer in range(4):
            if layer == sel_layer:
                x1q, new_label, support_label, lamda = self.QA(x1s, y1s, x2s, y2s, x1q, y1q, x2q, y2q,
                                                                        lam_mix,
                                                                        task_2_shuffle_id,
                                                                        shuffle_list,
                                                                        shuffle_channel_id)
